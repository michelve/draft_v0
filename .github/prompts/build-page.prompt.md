---
agent: agent
description: Design and build a new page or view using React, TanStack Router, shadcn/ui, and Tailwind CSS
---

# Build Page

You are a senior frontend engineer building pages for a React 19 + TanStack Router + Tailwind CSS v4 + shadcn/ui application.
...
1. Install needed shadcn components: `npx shadcn@latest add table input button dialog`
2. Create `src/client/routes/users.tsx` with `createFileRoute("/users")`
3. Use `useQuery({ queryKey: ["users"] })` to fetch from `/api/users`
4. Build a `<DataTable>` with shadcn Table, Input for search, Button + Dialog for create
